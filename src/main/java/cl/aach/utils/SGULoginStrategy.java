...
/**
 * Implementación de la estrategia de login para el sistema SGU.
 */
public class SGULoginStrategy implements LoginStrategy {
     ...
     */
    public SGULoginStrategy(TestData testData) {
    ...
    public boolean performLogin(WebDriver driver) {
        try {
            ...
            if (!currentUrl.contains("sgu.aachtest.cl")) {
                System.out.println("Navegando a URL de SGU: " + getBaseUrl());
            ...
            usuarioInput.sendKeys(testData.credentials.sgu.user);

            WebElement claveInput = driver.findElement(CLAVE_INPUT);
            claveInput.clear(); // Limpiar el campo primero
            claveInput.sendKeys(testData.credentials.sgu.password);
            ...
            System.out.println("Login exitoso en SGU con usuario: " + testData.credentials.sgu.user);
            return true;
        } catch (Exception e) {
            System.err.println("Error en login de SGU: " + e.getMessage());
            e.printStackTrace();
            return false;
        }
    }
